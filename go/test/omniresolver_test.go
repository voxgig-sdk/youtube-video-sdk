...
import (
	...
	omni "github.com/voxgig-sdk/youtube-video-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.YoutubeVideoError); is {
				out := map[string]any{
					"name":    "YoutubeVideoError",
...
func omniCtx(arg any, client *sdk.YoutubeVideoSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.YoutubeVideoSDK); is {
			client = live
