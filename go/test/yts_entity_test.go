...
func TestYtsEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		ytsRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.yts")))
...
func ytsBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("YOUTUBE_VIDEO_TEST_YTS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["YOUTUBE_VIDEO_TEST_YTS_ENTID"])
