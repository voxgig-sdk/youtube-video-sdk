...
//     (\`new YoutubeVideoSDK(...)\` / \`YoutubeVideoSDK.test(...)\` become
//     \`YoutubeVideoSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'YoutubeVideoSDK'
...
const TEST_SEED = {"entity":{"yts":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new YoutubeVideoSDK(...)\` and any
// \`YoutubeVideoSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+YoutubeVideoSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/YoutubeVideoSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
