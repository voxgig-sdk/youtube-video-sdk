# YoutubeVideo TypeScript SDK Reference

Complete API reference for the YoutubeVideo TypeScript SDK.
...
```ts
new YoutubeVideoSDK(options?: object)
```
...
#### `YoutubeVideoSDK.test(testopts?, sdkopts?)`
...
**Returns:** `YoutubeVideoSDK` instance in test mode.
...
Create a new `Yts` entity instance.
...
Alias for `YoutubeVideoSDK.test()`.
...
| `views` | `number` | Yes | Number of views the video has received |
...
```ts
const result = await client.Yts().load({ text: 'text' })
```
...
Return the parent `YoutubeVideoSDK` instance.
