...
local YtsEntity = {}
YtsEntity.__index = YtsEntity


function YtsEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, YtsEntity)
  self._name = "yts"
  self._client = client
...
function YtsEntity:get_name()
  return self._name
end


function YtsEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return YtsEntity.new(self._client, opts)
end
...
function YtsEntity:mark_deleted()
  self._deleted = true
end


function YtsEntity:deleted()
  return true == self._deleted
end


function YtsEntity:data_set(args)
  if args ~= nil then
...
function YtsEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function YtsEntity:match_set(args)
  if args ~= nil then
...
function YtsEntity:stream(action, args, callopts)
...
function YtsEntity:_run_op(ctx, post_done)
  local utility = self._utility
