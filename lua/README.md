# YoutubeVideo Lua SDK



The Lua SDK for the YoutubeVideo API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Yts()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/youtube-video-sdk/releases)),
...
Create an instance: `local yts = client:Yts(nil)`
...
```
lua/
├── youtube-video_sdk.lua    -- Main SDK module
...
The main module (`youtube-video_sdk`) exports the SDK constructor
