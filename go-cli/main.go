// Command YoutubeVideo-cli is a CLI + REPL for the YoutubeVideo SDK,
...
const prompt = "youtube-video"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "yts"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      YOUTUBE_VIDEO_APIKEY (api key), YOUTUBE_VIDEO_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: YOUTUBE_VIDEO_APIKEY carries the API key and
	// YOUTUBE_VIDEO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("YOUTUBE_VIDEO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("YOUTUBE_VIDEO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewYoutubeVideoSDK(opts)
