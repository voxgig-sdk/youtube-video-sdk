...
module YoutubeVideoUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = YoutubeVideoResponse.new({})
    ...
    if fetch_err
      ...
      response = YoutubeVideoResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = YoutubeVideoResponse.new(fetched)
    else
