...
require_relative 'YoutubeVideo_types'
...
class YoutubeVideoSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = YoutubeVideoHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = YoutubeVideoHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, YoutubeVideoFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    YoutubeVideoUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = YoutubeVideoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = YoutubeVideoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = YoutubeVideoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = YoutubeVideoHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = YoutubeVideoError.new(
        "graphql_error", "YoutubeVideoSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = YoutubeVideoSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
