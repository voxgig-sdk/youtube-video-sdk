...
class YtsEntity
  ...
  def make
    opts = @_entopts.dup
    YtsEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = YoutubeVideoHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = YoutubeVideoHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.Yts.load works with no args).
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = YoutubeVideoHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
