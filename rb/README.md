# YoutubeVideo Ruby SDK



The Ruby SDK for the YoutubeVideo API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Yts` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = YoutubeVideoSDK.test(testopts, sdkopts)
```
...
raise a `YoutubeVideoError` (a `StandardError` subclass), so wrap
...
```
rb/
├── YoutubeVideo_sdk.rb       -- Main SDK module
...
The main module (`YoutubeVideo_sdk`) exports the SDK class
