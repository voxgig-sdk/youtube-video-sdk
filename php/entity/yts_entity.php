...
class YtsEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new YtsEntity($this->_client, $opts);
    }

    /**
     * @param Yts|array $args Yts data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = YoutubeVideoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Yts|array The current Yts data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Yts fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = YoutubeVideoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Yts fields).
     */
     ...
     * Load a single Yts.
     *
     * @param YtsLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed YtsLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Yts|array The loaded Yts as an assoc-array at the
     *   SDK boundary; throws YoutubeVideoError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = YoutubeVideoHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
