...
// YoutubeVideo SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class YoutubeVideoTestFeature extends YoutubeVideoBaseFeature
{
    ...
    public function init(YoutubeVideoContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (YoutubeVideoContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = YoutubeVideoParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (YoutubeVideoContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(YoutubeVideoContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = YoutubeVideoParam::call($ctx, $k);
