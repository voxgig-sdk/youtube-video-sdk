...
// YoutubeVideo SDK retry feature
...
class YoutubeVideoRetryFeature extends YoutubeVideoBaseFeature
{
    ...
    public function init(YoutubeVideoContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (YoutubeVideoContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(YoutubeVideoContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(YoutubeVideoContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
