...
[MCP](https://modelcontextprotocol.io) server exposing the YoutubeVideo SDK as
two agent tools — `youtube-video_list` and `youtube-video_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/youtube-video-mcp)
make build
...
claude mcp add --scope user youtube-video \
  -- /absolute/path/to/youtube-video-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./youtube-video-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// youtube-video_list: first page of records
{ "entity": "yts" }
{ "entity": "yts", "query": { } }

// youtube-video_load: one record by id
{ "entity": "yts", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/youtube-video-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user youtube-video \
     -- "$PWD"/dist/*/youtube-video-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `youtube-video_list` and `youtube-video_load` tools now appear
   in new sessions. Ask the agent to *"list yts using youtube-video"*
   and it calls `youtube-video_list` with `{"entity":"yts"}`.
...
### Call the `youtube-video_list` tool
...
### Call the `youtube-video_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `youtube-video_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `youtube-video_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./youtube-video-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"youtube-video_load","arguments":{"entity":"yts","query":{"id":1}}}}'
```
