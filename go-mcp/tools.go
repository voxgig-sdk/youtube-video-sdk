...
type Args struct {
	Entity string         `json:"entity" jsonschema:"yts"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.YoutubeVideoSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "youtube-video_list",
		Description: "List records from YoutubeVideo. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "youtube-video_load",
		Description: "Load a single record from YoutubeVideo. " +
...
func runOp(client *sdk.YoutubeVideoSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.YoutubeVideoSDK, name string) (sdk.YoutubeVideoEntity, error) {
	switch strings.ToLower(name) {
	case "yts":
		return client.Yts(nil), nil
