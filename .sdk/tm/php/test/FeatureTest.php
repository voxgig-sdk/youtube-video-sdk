...
class FtCtrl extends YoutubeVideoControl
{
...
class FtClient
{
    ...
    private YoutubeVideoUtility $utility;

    public function __construct(YoutubeVideoUtility $utility, array $options)
    {
    ...
    public function get_utility(): YoutubeVideoUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (YoutubeVideoContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public YoutubeVideoContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (YoutubeVideoContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = YoutubeVideoConfig::make_feature($name);
    ...
    private static function buildUrl(YoutubeVideoSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof YoutubeVideoError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new YoutubeVideoResponse($fetched);
        }
    ...
    private function fail(YoutubeVideoContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (YoutubeVideoContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
