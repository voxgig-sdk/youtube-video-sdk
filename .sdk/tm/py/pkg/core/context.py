...
from projectname_sdk.core.control import YoutubeVideoControl
...
class YoutubeVideoContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, YoutubeVideoSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, YoutubeVideoResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, YoutubeVideoResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return YoutubeVideoError(code, msg, self)
