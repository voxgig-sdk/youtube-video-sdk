...
from projectname_sdk.core.operation import YoutubeVideoOperation
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return YoutubeVideoError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = YoutubeVideoSDK.test(None, None)
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(YoutubeVideoBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = YoutubeVideoSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = YoutubeVideoSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = YoutubeVideoOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = YoutubeVideoSDK.test(None, setup_opts)
