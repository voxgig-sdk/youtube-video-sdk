...
class YoutubeVideoTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            YoutubeVideoTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = YoutubeVideoTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = YoutubeVideoTestRunner.getenv("YOUTUBE_VIDEO_TEST_LIVE")
        override = YoutubeVideoTestRunner.getenv("YOUTUBE_VIDEO_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = YoutubeVideoTestRunner.getenv(key)
        ...
        explain = YoutubeVideoTestRunner.getenv("YOUTUBE_VIDEO_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if YoutubeVideoTestRunner._test_control is not None:
            return YoutubeVideoTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                YoutubeVideoTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            YoutubeVideoTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = YoutubeVideoTestRunner.load_test_control()
...
def load_env_local():
    YoutubeVideoTestRunner.load_env_local()


def env_override(m):
    return YoutubeVideoTestRunner.env_override(m)


def entity_data(v):
    return YoutubeVideoTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return YoutubeVideoTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return YoutubeVideoTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return YoutubeVideoTestRunner.load_test_control()


def live_client_options():
    return YoutubeVideoTestRunner.live_client_options()


def live_delay_ms():
    return YoutubeVideoTestRunner.live_delay_ms()
